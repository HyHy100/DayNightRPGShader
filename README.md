...
Video export creates an isolated fixed-resolution WebGL renderer, composites typography on a recording canvas, and uses `canvas.captureStream()` plus `MediaRecorder`. VP9 WebM is preferred with VP8 WebM as fallback. The finalizer writes an explicit Matroska `Duration` value matching the chosen 15/30/60-second timeline; this prevents players that cannot infer duration from live-recorded clusters from showing an early false endpoint. Recording is real-time, local-only, contains no audio, and is unavailable in browsers that do not expose a compatible WebM encoder; no image or video data is uploaded.

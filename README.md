...
Video export creates an isolated fixed-resolution WebGL renderer and composites typography on an encoding canvas. WebCodecs is preferred and gives every frame an explicit microsecond presentation timestamp. Where WebCodecs is unavailable, MediaRecorder is used only to compress individually requested canvas frames; its wall-clock timestamps are discarded. A local demux/remux pass then places the encoded frames on a new uniform timeline and writes finite VP9/VP8 clusters, cadence metadata, and an exact 15/30/60-second Matroska duration from that same clock. Rendering speed can therefore change export completion time but can never stretch, skip, or accelerate any part of the saved day cycle. Export is local-only, silent, and requires either WebCodecs or MediaRecorder with manual canvas-frame submission; no image or video data is uploaded.

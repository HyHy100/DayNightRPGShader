...
Lunar grading is gated by both Moon visibility and solar depression: it begins below −6°, reaches full availability below −12°, and contributes at most about +0.45 EV for an unusually favorable full Moon. It slightly opens shadow separation and black density with a restrained silver-neutral response; it never multiplies the image blue or fabricates directional relighting. Without location the UI reports **LUNAR GEOMETRY UNAVAILABLE** and retains only the moonless artistic night baseline. Clouds, artificial light, aurora, eclipses, zodiacal light, scene normals, and scene depth are not modeled.
...
- **Solar geometry:** optional date/location-aware solar and lunar calculations, entirely on-device.
- **Physics:** located mode shows Sun and Moon elevation, separate clear-sky DNI/DHI/GHI in W/m², and a logarithmic lunar-lux plot. Reference mode is explicitly normalized and nonphysical and does not invent a Moon trajectory. The inspector exposes lunar phase, distance, air mass, transmission, ground illuminance, and contribution alongside the solar/atmospheric state.
